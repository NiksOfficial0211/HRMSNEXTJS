...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        const fdata = {
            ...
            proClientName: formData.get('project_client_name'),
            proManagerID: formData.get('project_manager_id'),
            teamLeadID: formData.get('team_lead_id'),
        ...
        const { data: projectData, error: taskError } = await supabase.from('leap_client_project')
            .insert({
                client_id: fdata.clientID,
                branch_id: fdata.branchID,
                project_name: fdata.projectName,
                project_client: fdata.proClientName,
                project_manager_id: fdata.proManagerID,
        ...
        return NextResponse.json({ status: 1, message: "Project Added Successfully", }, { status: apiStatusSuccessCode })
        }

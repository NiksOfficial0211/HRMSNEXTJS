...
// const [imgSrc, setImgSrc] = useState(staticIconsBaseURL + "/images/user/user.png");
...
//     console.log(getImageApiURL + "/uploads/" + nodeDatum.profile_pic);
    
//     const imageURL = nodeDatum?.profile_pic
//     ? getImageApiURL + "/uploads/" + nodeDatum.profile_pic
//     : staticIconsBaseURL + "/images/user/user.png";

//   // getSafeImageUrl(imageURL, staticIconsBaseURL + "/images/user/user.png")
//   //   .then(setImgSrc);
...
type TreeNode = any; // Simplify or type properly later

// ✅ Helper: check image validity before using it
const getSafeImageUrl = (url: string, fallback: string): Promise<string> => {
  return new Promise((resolve) => {
    const img = new Image();
    img.onload = () => resolve(url);
...
// ✅ Subcomponent to safely render image inside SVG
const SafeSvgImage = ({
  imageURL,
  fallback,
  x,
...
const OrganizationTreeZoomIssue = ({
  employeeHerarichy,
}: {
  employeeHerarichy: EmployeeORGHierarchyDataModel[];
}) => {
  ...
  const renderCustomNode = ({ nodeDatum }: any) => {
    ...
    return (
      <g onClick={() => handleNodeClick(nodeDatum)} style={{ cursor: "pointer" }}>
        ...
        {/* ✅ Safe image that auto-falls back */}
        <SafeSvgImage
          imageURL={imageURL}
          fallback={fallback}
          x={-180}
...
export default OrganizationTreeZoomIssue;
